...
```{r}
chdf10<-subset(SN10HM,hv105<18)
#ignore missing values
chdf10 <- chdf10[,colSums(is.na(chdf10))<nrow(chdf10)*0.8]
```
...
df10features["upper"]<-integer(44)
for (x in 1:11){
  df10features$upper[x]<-CIupper(df10features$proportion[x],nrow(opdf))
}
for (x in 12:22){
  df10features$upper[x]<-CIupper(df10features$proportion[x],nrow(nopdf))
}
for (x in 23:33){
  df10features$upper[x]<-CIupper(df10features$proportion[x],nrow(lmdf))
}
for (x in 34:44){
  df10features$upper[x]<-CIupper(df10features$proportion[x],nrow(lfdf))
}
# confidence intervals lower
df10features["lower"]<-integer(44)
for (x in 1:11){
  df10features$lower[x]<-CIlower(df10features$proportion[x],nrow(opdf))
}
for (x in 12:22){
  df10features$lower[x]<-CIlower(df10features$proportion[x],nrow(nopdf))
}
for (x in 23:33){
  df10features$lower[x]<-CIlower(df10features$proportion[x],nrow(lmdf))
}
for (x in 34:44){
  df10features$lower[x]<-CIlower(df10features$proportion[x],nrow(lfdf))
}
```
...
```{r}
# draw the barplot
df10fea1<-subset(df10features,df10features$question>2)
ggplot(data = df10fea1, mapping = aes(x=question,y = proportion, fill = orphanhood)) + geom_bar(stat = 'identity', position = 'dodge') + geom_errorbar( aes(ymin=lower, ymax=upper), width=0.4, colour="orange", alpha=0.6,position = position_dodge(.9))
```
